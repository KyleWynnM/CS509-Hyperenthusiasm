 ...
 */
 exports.lambdaHandler = async (event, context, callback) => {
    ...
    let find_Store = (Latitude, Longitude) => {
        return new Promise((resolve, reject) => {
            pool.query("SELECT store_name FROM Store WHERE SQRT((? - latitude)^2 + (? - longitude)^2) < 100;" ,[Latitude],[Longitude],(error, rows) => {
                if (error) {
                    return reject(error);
                } else if (rows.length === 0) {
                    return resolve("No stores were found")
                } 
            })
        })
    }

    let List_store = (store_name) => {
        return new Promise((resolve, reject) => {
            pool.query("SELECT * from Store WHERE store_name=?", [store_name], (error, rows) => {
                    if (error) { 
                        return reject(error); 
                    } else {
